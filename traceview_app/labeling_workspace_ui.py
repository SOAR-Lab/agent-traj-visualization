...
def _render_workspace_sidebar(
    *,
    ...
    st.sidebar.metric("Unlabeled", candidate_count - labeled_count)

    st.sidebar.divider()
    st.sidebar.caption("EXPORT")
    if st.sidebar.button("Send to overview", type="primary", width="stretch"):
...
def render_workspace_page() -> None:
    ...
    st.markdown(f"#### Label Table: {selected_family_label}")
    st.caption(
